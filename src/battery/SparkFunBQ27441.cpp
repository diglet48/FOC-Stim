...
bool BQ27441::BatteryInsertEnable()
{
    uint16_t opConfigRegister = opConfig();

	return (opConfigRegister & BQ27441_OPCONFIG_BIE);
}

bool BQ27441::setBatteryInsertEnable(bool enable)
{
	uint16_t oldOpConfig = opConfig();

	// Check to see if we need to update opConfig:
	if ((enable && (oldOpConfig & BQ27441_OPCONFIG_BIE)) ||
        (!enable && !(oldOpConfig & BQ27441_OPCONFIG_BIE)))
		return true;

	// Modify BATLOWN_EN bit of opConfig:
	uint16_t newOpConfig = oldOpConfig;
	if (enable)
		newOpConfig |= BQ27441_OPCONFIG_BIE;
	else
		newOpConfig &= ~(BQ27441_OPCONFIG_BIE);
...
bool BQ27441::batInsert()
{
	return executeControlWord(BQ27441_CONTROL_BAT_INSERT);
}

bool BQ27441::batRemove()
{
	return executeControlWord(BQ27441_CONTROL_BAT_REMOVE);
}
